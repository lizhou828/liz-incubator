...
<%
    ...
    String typeScope = "doc,docx,xls,xlsx,ppt,txt,zip,rar,gz,bz2";
    ...
    //typeScope = typeScope.replace("FILE/", "").toUpperCase();
    //typeScope = typeScope.replace("MEDIA/", "").toUpperCase();
			...
			.contains(fileExt.toUpperCase()))
	{
		out.println(getError("上传文件扩展名是不允许的扩展名。\n只允许"
				+ typeScope.substring(0, typeScope.length() / 2) + "\n"
				+ typeScope.substring(typeScope.length() / 2) + "格式。"));
		return;
	}
	//检查文件大小  
	if (file.length() > maxSize)
	{
		out.println(getError("上传文件大小超过限制。文件大于" + ((float)maxSize/1024000) + "M！"));
		return;
	}
	//重构上传图片的名称   
