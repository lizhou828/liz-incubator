...
<%@page import="com.shangkang.bo.SupplyDemand"%>
<%@page import="org.apache.commons.fileupload.servlet.ServletFileUpload"%>
<%@page import="com.shangkang.utils.GraphicsMagick"%>
<%@page import="org.apache.struts2.ServletActionContext"%>
<%@page import="com.shangkang.bo.CustQual"%>
...
<%@page import="com.shangkang.bo.CompanyDynamic"%>
...
<%
	...
	if(!UtilHelper.isEmpty(request.getParameter("markTag")))
		markTag = Boolean.parseBoolean(request.getParameter("markTag"));
	if(!UtilHelper.isEmpty(request.getParameter("seat")))
		seat = Integer.parseInt(request.getParameter("seat"));
	if(!UtilHelper.isEmpty(request.getParameter("alpha")))
		alpha = Float.parseFloat(request.getParameter("alpha"));
	if(!UtilHelper.isEmpty(request.getParameter("degree")))
	...
	try
	{
		...
		if(markTag)
			GraphicsMagick.pressImage(PRESS_IMG, newImgSavePath, seat, alpha, degree);
		
		if(FileUpload.MOD_QUAL.equals(moduleid)){
			GraphicsMagick.resize(newImgSavePath, CustQual.QUAL_IMG_CENTER_SUFFIX, 200, 180);
		}else if(FileUpload.MOD_SUPPLY_DEMAND.equals(moduleid)){
			GraphicsMagick.resize(newImgSavePath, SupplyDemand.SUPPLY_DEMAND_IMG_CENTER_SUFFIX, 150, 150);
		}else if(FileUpload.MOD_DYNA.equals(moduleid)){
			GraphicsMagick.resize(newImgSavePath, CompanyDynamic.COMPANY_DYNA_IMG_CENTER_SUFFIX, 150, 150);
		}
